...
TMultiGraph* Pu239vsWeek(int det, int start_week=0, int end_week=187, 
		    bool add_old_SNF=1, double reactor_off_scaling=2.0)
{
  ...
  for(int iweek=start;iweek<=end;++iweek){
    ...
    for(int icore=0;icore<nCORE;++icore){
      ...
      double p = snf.ReactorPower(icore, iweek);
  ...
  mg->Draw("alp");
  mg->SetTitle(Form("Effective ^{239}Pu Fuel Fraction vs Week in P15A at %s", dname[det].Data()));
  gPad->Update();
  mg->GetXaxis()->SetTitleSize(0.036);
  mg->GetYaxis()->SetTitleSize(0.036);
  mg->GetYaxis()->SetMaximum(36);
  mg->GetYaxis()->SetMaximum(22);
  mg->GetYaxis()->SetTitle("Effective fraction of ^{239}Pu (%)");
  mg->GetXaxis()->SetTitle("Week since Dec 24, 2011");
  gPad->SetGrid();
  gPad->Update();
  return mg;
}
