...
TGraph* SNFdecaySingleRefuel(int ndays=21, int refuel=0, double lowE=1.806, double highE=9, 
		 bool add_old_SNF=1, double reactor_off_scaling=2.0)
{
  ...
  gr->SetLineStyle(9);
  gr->SetLineWidth(2);
  gr->SetTitle(Form("Average EH1 Fractional SNF Contribution for %i Days after Refuel %i for DB%i", ndays, (refuel>2 ? refuel-2:refuel+1),(refuel>2 ? 2:1)));
  gr->Draw("alp");
  gr->GetXaxis()->SetTitle("Days Since Refuel");
  gr->GetYaxis()->SetTitle("EH1 Fractional SNF Contribution (%)");
  gr->GetXaxis()->SetTitleSize(0.04);
  gr->GetYaxis()->SetTitleSize(0.04);
  gr->GetYaxis()->SetRangeUser(0.68, 1.2);
  ...
  pt->Draw();
  gPad->Update();
  gPad->SaveAs(Form("EH1_SNF_first21days_refuel%i.png",refuel+1));
  return gr;
}
